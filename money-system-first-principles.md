...
三条都从追问里长出来，不用另起一章。一张图看清楚：

```mermaid
flowchart LR
    subgraph 追问["三个追问"]
        Q1["① 钱是什么？<br/>→ 对社会产出的索取权<br/>（定义基础，不直接给行动）"]
    ...
    subgraph 推导["逻辑推导"]
        D1["A + B"]
        D2["增长一直在<br/>但恐慌让人跑偏"]
        D3["时间-流动性错配<br/>浮亏→被迫卖→永久亏损"]
    end

    subgraph 结论["三个行动结论"]
        C1["<b>买整个市场</b><br/>不赌个别公司<br/>（概率推论）"]
        C2["<b>用规则替代判断</b><br/>在恐慌时替你留在原地<br/>（行为法则）"]
        C3["<b>短钱和长钱分开放</b><br/>防火墙保护复利<br/>（逻辑公理）"]
    end
...
1. **结论一不是从一个追问来的——是两个追问的交点。** 追问 2 说增长是真的，追问 3 说我挑不中。两个合在一起才推得出"那就都买"。
2. **结论二也是两个追问的交点。** 追问 2 说增长一直在——那为什么很多人没赚到？追问 3 给了原因：恐慌时人会跑。所以需要一个东西在人想跑的时候替他留在原地。
3. **结论三只从追问 3 的引申里来。** 它不是概率问题——是时间错配了，浮亏就要变真亏，这是逻辑必然，不需要数据验证。画成虚线，因为它和前两条性质不同。
