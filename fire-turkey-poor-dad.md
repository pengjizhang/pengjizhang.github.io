...
```mermaid
graph TB
    subgraph 高防御性×高不可替代性
        I["🏗️ I象限 投资人<br/>系统设计者"]
    end
    subgraph 低防御性×高不可替代性
        B["🌾 B象限 企业主<br/>农夫"]
    end
    subgraph 低防御性×低不可替代性
        E["🐔 E象限 雇员<br/>火鸡"]
    end
    subgraph 高防御性×低不可替代性
        S["🔧 S象限 自雇者<br/>半火鸡"]
    end
    E -->|"进化"| S -->|"进化"| B -->|"进化"| I
...
```mermaid
graph LR
    A["🐔 穷爸爸火鸡<br/>E象限：工资→交税→消费<br/>→存钱→房贷→等退休"] 
    -->|"防御：分清资产和负债<br/>先付自己，建现金流"| B["👀 觉醒火鸡<br/>多条收入线<br/>六个月现金储备"]
    B -->|"进攻：E→S→B<br/>用业余时间建收入系统"| C["🌾 农夫<br/>企业/资产替你赚钱<br/>税前抵扣，税后投资"]
    C -->|"共生：B→I<br/>用杠杆，走四条车道"| D["🏗️ 系统设计者<br/>资产替你赚钱<br/>税只是成本项<br/>借钱不交税"]
